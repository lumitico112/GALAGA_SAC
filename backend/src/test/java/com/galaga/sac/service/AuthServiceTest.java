...
public class AuthServiceTest {
    ...
    void setUp() {
        ...
        mockUsuario.setEmail("test@galaga.com");
        mockUsuario.setPassword("encoded_password");
        mockUsuario.setRol("ADMIN");
        mockUsuario.setEstado("Activo");
    }

    @Test
    void testLoginSuccess() {
        LoginRequest req = new LoginRequest();
        req.setEmail("test@galaga.com");
        req.setPassword("123456");

        Authentication mockAuth = mock(Authentication.class);
        when(authenticationManager.authenticate(any(UsernamePasswordAuthenticationToken.class))).thenReturn(mockAuth);
        when(tokenProvider.generateToken(mockAuth)).thenReturn("mock_jwt_token");
        when(usuarioRepository.findByEmail("test@galaga.com")).thenReturn(Optional.of(mockUsuario));

        AuthResponse res = authService.login(req);

        assertNotNull(res);
        assertEquals("mock_jwt_token", res.getToken());
        assertEquals("test@galaga.com", res.getEmail());
    ...
    void testRegisterSuccess() {
        ...
        Usuario res = authService.register(req);

        assertNotNull(res);
        assertEquals("nuevo@galaga.com", res.getEmail());
